...
async function boot() {
  ...
  subscribeCases(
    (rows) => {
      ...
      updateBadges();
      hideSplash(); // 初回データ到着 → 起動画面をフェードアウト（最低表示時間はスプラッシュ側で担保）
    },
    (err) => { state.loading = false; state.authError = true; renderRoute(); hideSplash(); }
  );
  ...
  try {
    ...
    renderRoute();
    hideSplash(); // エラー表示が起動画面に隠れないよう必ず消す
  }
}

// ---- 起動画面（スプラッシュ） -------------------------------
// 実体は index.html のインラインスクリプト（window.hideSplash）。
// 読み込みが早くても最低0.8秒表示・8秒で強制消去はそちらで担保している。
function hideSplash() {
